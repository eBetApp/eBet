...
import { dispatchUserNull, dispatchUserAccountBalanceNull } from '../../Redux/dispatchers';
// UI imports
...
export default function LoggedScreen({ navigation }: LoggedScreenProps) {
	...
	const {
		fetch: fetchBalance,
		fetchIsProcessing: fetchBalanceIsLoading,
	...
	const _renderLoggedView = () => (
		<MainKeyboardAvoidingView style={styles.mainContainer}>
			<ScrollView
				...
				refreshControl={
					<RefreshControl refreshing={fetchBalanceIsLoading} onRefresh={fetchBalance} />
				}
			>
				...
				/>
				<TouchableOpacity onPress={() => navigation.navigate(Navigation.Screens.password)}>
					<Input
						editable={false}
				...
				<View style={styles.bottomContainer}>
					<View style={styles.buttonContainer}>
						<ButtonEdit
							...
							onPress={fetchEdit}
							icon={
								<Icon name="edit" type="font-awesome" style={{ marginRight: 5 }} />
							}
						/>
					</View>
					<View style={styles.buttonContainer}>
						<ButtonValid
							...
							icon={
								<Icon name="logo-euro" type="ionicon" style={{ marginRight: 5 }} />
							}
						/>
					</View>
				</View>
				<View style={styles.buttonContainer}>
					<ButtonCancel
						...
						icon={
							<Icon name="sign-out" type="font-awesome" style={{ marginRight: 5 }} />
						}
					/>
				</View>
			</ScrollView>
	...
	return <>{state.user?.accountId != null ? _renderLoggedView() : _renderWebView()}</>;
}
