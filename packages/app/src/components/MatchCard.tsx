...
export function MatchCard(props: IProps) {
	...
	const betOnTeam = async (teamId: number) => {
				...
				<View style={{ flex: 1, flexDirection: "row" }}>
					{
						canBet ? (
							<>
								<View style={styles.equipeCanBet} onTouchEnd={() => betOnTeam(match.opponents[0]?.opponent.id)} >
									<Text>{match.opponents[0]?.opponent.name}</Text>
									<Text>Odd : {match.opponents[0]?.opponent.odd}</Text>
								</View>
								<View style={styles.equipeCanBet} onTouchEnd={() => betOnTeam(match.opponents[1]?.opponent.id)} >
